...
import { Plus, Archive, Trash2 } from 'lucide-vue-next'
...
async function removeProduction(production: Production) {
  const ok = await confirm({
    ...
    message: 'Its numbers, cast, groups, and roles will be deleted too.',
    destructive: true,
    confirmText: 'Delete',
  })
  if (!ok) return
...
<template>
  <div class="mx-auto max-w-3xl space-y-6 p-6">
    <div>
      <h1 class="font-display text-2xl font-bold">Seasons &amp; Productions</h1>
    ...
    >
      <div class="flex items-center justify-between border-b border-border px-4 py-3">
        <div>
          ...
          <span class="ml-2 text-xs text-muted-foreground">{{ season.year }}</span>
          <span
      ...
      <form
        ...
        @submit.prevent="addProduction(season)"
      >
        <input
          v-model="newProductionTitle[season.id]"
          placeholder="New production title (e.g. Annie)"
